...
void RenderViewImpl::didFinishDocumentLoad(WebFrame* frame) {
  ...
  document_state->set_finish_document_load_time(Time::Now());
	...
	//fprintf(stderr, "RenderViewImpl::didFinishDocumentLoad %lld %s\n", frame->identifier(), ds->request().url().spec().data());//Sophia
...
void RenderViewImpl::didHandleOnloadEvents(WebFrame* frame) {
  if (webview()->mainFrame() == frame) {
	  // This happens uniquely per page while didFinishDocumentLoad() and didFinishLoad() happens per frame.
	  WebDataSource* ds = frame->dataSource();//Sophia
	  fprintf(stderr, "RenderViewImpl::didHandleOnloadEvents %lld %s\n", frame->identifier(), ds->request().url().spec().data());//Sophia
...
void RenderViewImpl::didFinishLoad(WebFrame* frame) {
  ...
  FOR_EACH_OBSERVER(RenderViewObserver, observers_, DidFinishLoad(frame));
	...
	//fprintf(stderr, "RenderViewImpl::didFinishLoad %lld %s\n", frame->identifier(), ds->request().url().spec().data());//Sophia
