...
void PopulateURLResponse(
	...
	if (url.spec().c_str() != NULL && url.spec().c_str()[0] == 'h') {
	/*fprintf(stderr, "{\"ResourceInfoChrome\":{\"url\":\"%s\", \"conn_id\":%d, \"conn_reused\":%d, \"spdy\":%d, \"npn\":%d,\
			\"proxy_start\":%d, \"proxy_end\":%d, \"dns_start\":%d, \"dns_end\":%d, \"connect_start\":%d, \"connect_end\":%d,\
			\"ssl_start\":%d, \"ssl_end\":%d, \"send_start\":%d, \"send_end\":%d, \"receive_headers_end\":%d}}\n",
			url.spec().c_str(),
			info.connection_id,
			info.connection_reused,
			info.was_fetched_via_spdy,
			info.was_npn_negotiated,
			info.load_timing.proxy_start,
			info.load_timing.proxy_end,
			info.load_timing.dns_start,
			info.load_timing.dns_end,
			info.load_timing.connect_start,
			info.load_timing.connect_end,
			info.load_timing.ssl_start,
			info.load_timing.ssl_end,
			info.load_timing.send_start,
			info.load_timing.send_end,
